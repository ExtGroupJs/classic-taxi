...
function getmodalityid(name) {
  let url =
    "../event-gestion/participation-modality-language/?page_size=1&search=" +
    name;
  axios
    .get(`${url}`)
    .then((response) => {
    ...
    .catch((error) => {
      alert(
        "revisa el nombre que estas pasando en la creación de la modalidad contra la función"
      );
    });
}
const url2 = "../event-gestion/participation-modality-language/";
